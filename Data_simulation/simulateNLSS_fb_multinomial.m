function [y,x,x0,eta,e,u] = simulateNLSS_fb_multinomial(n_t,f_fname,g_fname,theta,phi,u,alpha,sigma,options,x0,fb)
...
if ~options.binomial
    ...
    for i=1:dim.p
        y(i,1) = sampleFromArbitraryP([gt],[1:Na],1);
    end
    e(:,1) = y(:,1) - gt(y(1,1));
end
...
for t = 2:dim.n_t
    ...
    if ~options.binomial
        ...
        for i=1:dim.p
            y(i,t) = sampleFromArbitraryP([gt],[1:Na],1);
        end
        e(:,t) = y(:,t) - gt(y(1,t));
    end
