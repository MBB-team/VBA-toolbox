...
out.hf = figure('position',pos,'color',[1 1 1],...
    'name',['1D-RFT analysis (set-inducing thresholds: X>',num2str(options.u,'%3.2f'),', k>',num2str(options.k),')'],...
    'tag','RFT1D');
...
for i=1:nc
    ...
    uimenu(hcmenu, 'Label',['position: ',num2str(min(clusters.ind{i})),'<t<',num2str(max(clusters.ind{i}))]);
...
for i=1:length(peaks.ind)
    ...
    uimenu(hcmenu, 'Label',['position: t=',num2str(peaks.ind(i))]);
...
xlabel(ha,'position on the lattice (t)')
