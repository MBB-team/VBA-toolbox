function [ix] = VBA_spm_fieldindices(X,varargin)
   ...
   if isnumeric(varargin{1})
       if numel(varargin{1}) > 1
           for j = 1:length(varargin{1})
               ix{j} = VBA_spm_fieldindices(X,varargin{1}(j));
           end
           return
       end
   end
...
for i = 1:length(varargin)
    ...
    elseif isnumeric(varargin{1})
        name  = fieldnames(X);
        for j = 1:length(name)
            k = VBA_spm_fieldindices(X,name{j});
