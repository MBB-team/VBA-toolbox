function VBA_spm_dcm_display(varargin)
...
if n < 4
    Fgraph  = VBA_spm_figure('GetWin','Graphics');
    ha = gca;
else
    ha = varargin{4};
...
% get T1 background
%--------------------------------------------------------------------------
V       = VBA_spm_vol(fullfile(spm('Dir'),'canonical','single_subj_T1.nii'));
ijk     = V.mat \ xyz;
t1      = VBA_spm_sample_vol(V,ijk(1,:),ijk(2,:),ijk(3,:),2);
