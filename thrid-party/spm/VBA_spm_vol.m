...
elseif iscell(P)
    V = cellfun(@VBA_spm_vol,P, 'UniformOutput',false);
    
else
    V = VBA_spm_vol;
    cnt = 0;
    for i=1:size(P,1)
        v = VBA_spm_vol_hdr(deblank(P(i,:)));
...
%==========================================================================
function V = VBA_spm_vol_hdr(p)
[pth,nam,ext,n] = VBA_spm_fileparts(p);
p = fullfile(pth,[nam ext]);
n = str2num(n);
if ~VBA_spm_existfile(p)
...
switch ext
    ...
    case {'.img','.IMG'}
        if ~VBA_spm_existfile(fullfile(pth,[nam '.hdr'])) && ...
           ~VBA_spm_existfile(fullfile(pth,[nam '.HDR']))
    ...
    case {'.gz','.GZ'}
        ...
        try
            ...
            V = VBA_spm_vol_hdr([tmpname n]);
            for i=1:numel(V)
                V(i).dat   = VBA_spm_read_vols(V(i));
                V(i).private.dat.fname = VBA_spm_file(p,'ext','');
                V(i).fname = p;
                V(i).dt(1) = 64;
                V(i).pinfo = [1 0 0]';
            end
