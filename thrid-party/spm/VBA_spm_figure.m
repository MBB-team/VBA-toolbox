function varargout=VBA_spm_figure(varargin)
...
F = VBA_spm_figure('CreateWin',Tag,Name,Visible);
VBA_spm_figure('CreateBar',F);
...
F = VBA_spm_figure('FindWin',Tag);

if isempty(F)
    if ischar(Tag)
        switch Tag
            case 'Graphics'
                F = VBA_spm_figure('Create','Graphics','Graphics');
            case 'DEM'
                F = VBA_spm_figure('Create','DEM','Dynamic Expectation Maximisation');
            case 'DFP'
                F = VBA_spm_figure('Create','DFP','Variational filtering');
            case 'FMIN'
                F = VBA_spm_figure('Create','FMIN','Function minimisation');
            case 'MFM'
                F = VBA_spm_figure('Create','MFM','Mean-field and neural mass models');
            case 'MVB'
                F = VBA_spm_figure('Create','MVB','Multivariate Bayes');
            case 'SI'
                F = VBA_spm_figure('Create','SI','System Identification');
            case 'PPI'
                F = VBA_spm_figure('Create','PPI','Physio/Psycho-Physiologic Interaction');
            case 'Interactive'
                F = spm('CreateIntWin');
            case 'Satellite'
                F = VBA_spm_figure('CreateSatWin');
            otherwise
                F = VBA_spm_figure('Create',Tag,Tag);
        end
    end
else
    figure(F);
...
case 'parentfig'
%==========================================================================
% F=VBA_spm_figure('ParentFig',h)

warning('VBA_spm_figure(''ParentFig'',h) is deprecated. Use ANCESTOR instead.');
...
case 'clear'
%==========================================================================
% VBA_spm_figure('Clear',F,Tags)

%-Sort out arguments
if nargin<3, Tags=[]; else Tags=varargin{3}; end
if nargin<2, F=get(0,'CurrentFigure'); else F=varargin{2}; end
F = VBA_spm_figure('FindWin',F);
...
hMenu = VBA_spm_figure('FindWin','Menu');
...
case 'print'
%==========================================================================
% VBA_spm_figure('Print',F,fname,opt)

%-Arguments & defaults
if nargin<4, opt=VBA_spm_get_defaults('ui.print'); else opt=varargin{4}; end
...
if ~isempty(F), F = VBA_spm_figure('FindWin',F); end
...
def = VBA_spm_get_defaults('ui.print');
if ischar(def), [def, i] = VBA_spm_print('format',def); end
pf  = VBA_spm_print('format');
...
VBA_spm_figure('Print',F,fname,pf(fi));

%==========================================================================
case 'newpage'
%==========================================================================
% [hNextPage, hPrevPage, hPageNo] = VBA_spm_figure('NewPage',h)
...
if isempty(hNextPage)
    ...
    hNextPage = uicontrol(F,'Style','Pushbutton',...
        ...
        'Callback','VBA_spm_figure(''TurnPage'',''+1'',gcbf)',...
...
case 'turnpage'
%==========================================================================
% VBA_spm_figure('TurnPage',move,F)
...
%==========================================================================
% VBA_spm_figure('DeletePageControls',F)
...
case 'currentpage'
%==========================================================================
% n = VBA_spm_figure('CurrentPage', F)
...
case 'watermark'
%==========================================================================
% VBA_spm_figure('WaterMark',F,str,Tag,Angle,Perm)
...
text(0.5,0.5,str,...
    ...
    'FontWeight','Bold',...
    'FontName',VBA_spm_platform('Font','times'),...
    'Rotation',Angle,...
    'HorizontalAlignment','Center',...
...
PF   = VBA_spm_platform('fonts');     %-Font names (for this platform)
...
F    = figure(...
    ...
    'InvertHardcopy','off',...
    'Renderer',VBA_spm_get_defaults('renderer'),...
    'Visible','off',...
    'Toolbar','none');
if ~isempty(Name)
...
isdocked = strcmp(get(VBA_spm_figure('FindWin','Menu'),'WindowStyle'),'docked');
if isdocked
    try
        desktop = com.mathworks.mde.desk.MLDesktop.getInstance;
        group   = ['Statistical Parametric Mapping (' spm('Ver') ')'];
...
%==========================================================================
% F=VBA_spm_figure('CreateSatWin')
F = VBA_spm_figure('FindWin','Satellite');
if ~isempty(F)
    VBA_spm_figure('Focus', F)
else
    FS   = [2 3 4 5 6 8 9 10 11 12 13 15 16 17 18 19 21 22 23 24 25 26 28 29 30 31 32 34 35 36 37 38 39 41 42 43]; %spm('FontSizes');             %-Scaled font sizes
    PF   = VBA_spm_platform('fonts');        %-Font names
    WS   = get(0,'screensize');     %-Screen size (of current monitor)
...
pos = get(t0,'Position');
uimenu(t0,'Label','SPM Help','CallBack','VBA_spm_help');
...
if ~isdeployed
    uimenu(t0,'Separator','on','Label','SPM Check Installation',...
        'CallBack','VBA_spm_check_installation(''full'')');
    uimenu(t0,'Label','SPM Check for Updates',...
        'CallBack',@VBA_spm_check_update);
end

%- About Menu
uimenu(t0,'Separator','on','Label',['&About ' spm('Ver')],...
    'CallBack',@VBA_spm_about);
...
uimenu(t0,    'Label','&Save Figure', 'HandleVisibility','off', ...
    'CallBack','VBA_spm_figure(''Print'',gcf)', 'Separator','on');
uimenu(t0,    'Label','Save Figure &As...', 'HandleVisibility','off', ...
    'CallBack','VBA_spm_figure(''PrintTo'',gcf)');
...
uimenu(t0,    'Label','&Clear Figure', 'HandleVisibility','off', ...
    'CallBack','VBA_spm_figure(''Clear'',gcbf)');
...
uimenu(t2,    'Label','Gray',      'CallBack','VBA_spm_figure(''ColorMap'',''gray'')');
uimenu(t2,    'Label','Hot',       'CallBack','VBA_spm_figure(''ColorMap'',''hot'')');
uimenu(t2,    'Label','Pink',      'CallBack','VBA_spm_figure(''ColorMap'',''pink'')');
uimenu(t2,    'Label','Jet',       'CallBack','VBA_spm_figure(''ColorMap'',''jet'')');
uimenu(t2,    'Label','Gray-Hot',  'CallBack','VBA_spm_figure(''ColorMap'',''gray-hot'')');
uimenu(t2,    'Label','Gray-Cool', 'CallBack','VBA_spm_figure(''ColorMap'',''gray-cool'')');
uimenu(t2,    'Label','Gray-Pink', 'CallBack','VBA_spm_figure(''ColorMap'',''gray-pink'')');
uimenu(t2,    'Label','Gray-Jet',  'CallBack','VBA_spm_figure(''ColorMap'',''gray-jet'')');
t2=uimenu(t1, 'Label','Effects');
uimenu(t2,    'Label','Invert',    'CallBack','VBA_spm_figure(''ColorMap'',''invert'')');
uimenu(t2,    'Label','Brighten',  'CallBack','VBA_spm_figure(''ColorMap'',''brighten'')');
uimenu(t2,    'Label','Darken',    'CallBack','VBA_spm_figure(''ColorMap'',''darken'')');

%-Font Size Menu
t1=uimenu(t0, 'Label','&Font Size', 'HandleVisibility','off');
uimenu(t1, 'Label','&Increase', 'CallBack','VBA_spm_figure(''FontSize'',1)',  'Accelerator', '=');
uimenu(t1, 'Label','&Decrease', 'CallBack','VBA_spm_figure(''FontSize'',-1)', 'Accelerator', '-');
...
uimenu(t0,    'Label','&Results Table', 'HandleVisibility','off', ...
    'Separator','on', 'Callback','VBA_spm_figure(''CreateSatWin'');');
    
%==========================================================================
case 'figcontextmenu'
%==========================================================================
% h = VBA_spm_figure('FigContextMenu',F)

if nargin<2
    F = get(0,'CurrentFigure');
    if isempty(F), error('no figure'), end
else
    F = VBA_spm_figure('FindWin',varargin{2});
...
case 'colormap'
%==========================================================================
% VBA_spm_figure('ColorMap',ColAction)
...
case 'fontsize'
%==========================================================================
% VBA_spm_figure('FontSize',sz)

if nargin<2, sz=0; else sz=varargin{2}; end

h  = [get(0,'CurrentFigure') VBA_spm_figure('FindWin','Satellite')];
...
SatWindow = VBA_spm_figure('FindWin','Satellite');
...
hMenu = VBA_spm_figure('FindWin','Menu');
hInt  = VBA_spm_figure('FindWin','Interactive');
hGra  = VBA_spm_figure('FindWin','Graphics');
h     = [hMenu hInt hGra];
group   = ['Statistical Parametric Mapping (' spm('Ver') ')'];
try
    desktop = com.mathworks.mde.desk.MLDesktop.getInstance;
...
hInt  = VBA_spm_figure('FindWin','Interactive');
hGra  = VBA_spm_figure('FindWin','Graphics');
hSat  = VBA_spm_figure('FindWin','Satellite');
hBat  = VBA_spm_figure('FindWin','cfg_ui');
...
%==========================================================================
function VBA_spm_about(obj,evt)
%==========================================================================
...
cla(ax);
[current, previous] = VBA_spm_authors;
...
%==========================================================================
function VBA_spm_check_update(obj,evt)
%==========================================================================
[sts, msg] = VBA_spm_update;
