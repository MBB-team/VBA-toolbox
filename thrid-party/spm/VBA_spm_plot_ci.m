function VBA_spm_plot_ci(E,C,x,j,s)
...
%--------------------------------------------------------------------------
if isstruct(E),       E = VBA_spm_vec(E);    end
if iscell(E),         E = VBA_spm_vec(E);    end
...
%--------------------------------------------------------------------------
ci    = VBA_spm_invNcdf(1 - 0.05);               % confidence interval
gr    = 0.9;                                 % grey level
if iscell(C)
    ...
    try
        ...
        %------------------------------------------------------------------
        c = ci*sqrt(VBA_spm_unvec(spm_vec(C),E));
        c = c(j,:);
    end
