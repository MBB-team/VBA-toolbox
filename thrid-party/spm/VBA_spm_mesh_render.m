function varargout = VBA_spm_mesh_render(action,varargin)
...
switch lower(action)
    ...
    case 'disp'
        if isempty(varargin)
            [M, sts] = VBA_spm_select(1,'mesh','Select surface mesh file');
            if ~sts, return; end
        else
            M = varargin{1};
        end
        ...
        %------------------------------------------------------------------
        C = VBA_spm_mesh_label(P);
        setappdata(H.patch,'cclabel',C);
        
        %-Compute mesh curvature
        %------------------------------------------------------------------
        curv = VBA_spm_mesh_curvature(P) > 0;
        ...
        %-Add context menu
        %------------------------------------------------------------------
        VBA_spm_mesh_render('ContextMenu',H);
    ...
    case 'register'
        ...
        VBA_spm_XYZreg('Add2Reg',hReg,hs,@myCrossBar);
        
    %-Otherwise...
    %======================================================================
    otherwise
        try
            H = VBA_spm_mesh_render('Disp',action,varargin{:});
        catch
...
VBA_spm_mesh_inflate(H.patch,Inf,1);
axis(H.axis,'image');
...
VBA_spm_mesh_render('Colourbar',H,toggle(get(obj,'Checked')));

%==========================================================================
function myColourmap(obj,evt,H)
VBA_spm_mesh_render('Colourmap',H,feval(get(obj,'Label'),256));
...
if ~isempty(hMe)
    ws = warning('off');
    VBA_spm_XYZreg('SetCoords',pos,get(hMe,'UserData'));
    warning(ws);
end
...
[P, sts] = VBA_spm_select(1,'\.img|\.nii|\.gii|\.mat','Select file to overlay');
if ~sts, return; end
VBA_spm_mesh_render('Overlay',H,P);

%==========================================================================
function myImageSections(obj,evt,H)
[P, sts] = VBA_spm_select(1,'image','Select image to render');
if ~sts, return; end
renderSlices(H,P);
...
if ischar(v)
    ...
    if strcmp([n e],'SPM.mat')
        swd = pwd;
        VBA_spm_figure('GetWin','Interactive');
        [SPM,v] = VBA_spm_getSPM(struct('swd',p));
        cd(swd);
    else
        try, VBA_spm_vol(v); catch, v = gifti(v); end;
    end
end
...
elseif ischar(v) || iscellstr(v) || isstruct(v)
    v = VBA_spm_mesh_project(H.patch,v);
...
%--------------------------------------------------------------------------
if isfield(H,'colourbar')
    VBA_spm_mesh_render('Colourbar',H);
end
