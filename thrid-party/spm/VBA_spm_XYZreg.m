function varargout = VBA_spm_XYZreg(varargin)
...
%--------------------------------------------------------------------------
[d,i] = min(VBA_spm_XYZreg('Edist',varargin{2},varargin{3}));
...
%--------------------------------------------------------------------------
[xyz,d] = VBA_spm_XYZreg('RoundCoords',xyz,M,D);
if d>0 && nargout<2
...
case 'getcoords'                                  % Get current coordinates
 %=========================================================================
% xyz = VBA_spm_XYZreg('GetCoords',hReg)
if nargin<2, hReg=VBA_spm_XYZreg('FindReg'); else hReg=varargin{2}; end
...
%==========================================================================
% [xyz,d] = VBA_spm_XYZreg('SetCoords',xyz,hReg,hC,Reg)
...
%--------------------------------------------------------------------------
if hC<=0
    [xyz,d] = VBA_spm_XYZreg('RoundCoords',xyz,RD.M,RD.D);
    if d>0 && nargout<2
        warning('Coordinates rounded to nearest voxel center: Discrepancy %.2f',d);
    end
else
...
%--------------------------------------------------------------------------
if nargin<5
    RD.Reg = VBA_spm_XYZreg('VReg',RD.Reg);
    Reg    = RD.Reg;
else
    Reg = VBA_spm_XYZreg('VReg',varargin{5});
end
...
case 'xreg'                     % Cross register object handles & functions
 %=========================================================================
% nReg = VBA_spm_XYZreg('XReg',hReg,{h,Fcn}pairs)
if nargin<4, error('Insufficient arguments'), end
hReg = varargin{2};
...
nReg = VBA_spm_XYZreg('VReg',nReg,'Warn');

%-Set hReg registry link for registry candidates (Back registration)
%--------------------------------------------------------------------------
for i = 1:size(nReg,1)
    VBA_spm_XYZreg('SetReg',nReg{i,1},hReg);
end
...
%--------------------------------------------------------------------------
VBA_spm_XYZreg('SetCoords',RD.xyz,hReg,hReg,nReg);
...
%==========================================================================
% nReg = VBA_spm_XYZreg('Add2Reg',hReg,{h,Fcn}pairs)
if nargin<4, error('Insufficient arguments'), end
hReg = varargin{2};
...
%==========================================================================
% nReg = VBA_spm_XYZreg('unXReg',hReg,hD1,hD2,hD3,...)
if nargin<3, error('Insufficient arguments'), end
hD       = [varargin{3:end}];
hReg     = varargin{2};
...
hD(e)    = [];
dReg     = VBA_spm_XYZreg('VReg',Reg(i,:));
Reg(i,:) = [];
...
%--------------------------------------------------------------------------
for i = 1:size(dReg,1)
    VBA_spm_XYZreg('SetReg',dReg{i,1},[]);
end
...
%==========================================================================
% nReg = VBA_spm_XYZreg('Del2Reg',hReg,hD)
if nargin<3, error('Insufficient arguments'), end
hD       = [varargin{3:end}];
hReg     = varargin{2};
...
%==========================================================================
% VBA_spm_XYZreg('UnSetReg',h)
if nargin<2, error('Insufficient arguments'), end
VBA_spm_XYZreg('SetReg',varargin{2},[])
...
%==========================================================================
% VBA_spm_XYZreg('CleanReg',hReg)
%if ~strcmp(get(hReg,'Tag'),'hReg'), error('Not a registry'), end
hReg = varargin{2};
RD = get(hReg,'UserData');
RD.Reg = VBA_spm_XYZreg('VReg',RD.Reg,'Warn');
set(hReg,'UserData',RD)
...
%==========================================================================
% Reg = VBA_spm_XYZreg('VReg',Reg,Warn)
...
%==========================================================================
% hReg = VBA_spm_XYZreg('FindReg',h)
if nargin<2, h=get(0,'CurrentFigure'); else h=varargin{2}; end
if ischar(h), h=VBA_spm_figure('FindWin',h); end
