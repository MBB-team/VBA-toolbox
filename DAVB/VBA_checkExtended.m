function [options,u,dim] = VBA_checkExtended(varargin)
% checks (and fills in default) optional inputs to VB_NLStateSpaceModel.m
% function [options,u,dim] = VBA_check(y,u,f_fname,g_fname,dim,options) or
% function [options,u,dim] = VBA_check(y,r,u,f_fname,g_fname,h_fname,dim,options)
...
if nargin == 6 % full DCM
    ...
    if isfield(options,'extended') && options.extended
        error('VBA_check requires 8 arguments for extended DCM');
    end
    options.extended=false;
elseif nargin == 8 % extended DCM
    y = varargin{1};
    r = varargin{2};
    u = varargin{3};
    f_fname = varargin{4};
...
% Optional (internal) parameters of the decoding function


 
% ## TODO : check priors with VBA_priors
...
if options.extedned
    try
        ...
        error('Provide dimensions of the extended model!')
    end
end
...
% Optional (internal) parameters of the decoding function
if options.extended && ~isfield(options,'inH')
    options.inH = [];
end
...
if options.extended
    options.params2update.r0 = 1:dim.n_r;
    options.params2update.psi = 1:dim.n_psi;
end
for t=1:dim.n_t
