...
if isequal(options.g_fname,@VBA_odeLim) || ...
    ...
    if (~options.inG.old.options.priors.AR)  || ...
            (~flag && options.inG.old.options.priors.AR)  || ...
            isequal(options.g_fname,@VBA_smoothNLSS)
        % for non AR
        ...
        if dim.n_phi > 0
            ...
            posterior.SigmaPhi = ...
                posterior0.SigmaPhi(1:dim.n_phi,1:dim.n_phi);
        ...
        if dim.n_theta > 0
            posterior.muTheta = ...
                posterior0.muPhi(dim.n_phi+1:dim.n_phi+dim.n_theta);
            posterior.SigmaTheta = ...
                posterior0.SigmaPhi(dim.n_phi+1:dim.n_phi+dim.n_theta,...
            ...
            if ~isempty(indIn)
                suffStat.Stheta = 0.5*length(indIn)*log(2*pi*exp(1)) ...
                    + 0.5*VBA_logDet(posterior.SigmaTheta,indIn);
            else
                suffStat.Stheta = 0;
            end
        else
        ...
        if options.updateX0
            posterior.muX0 = ...
                posterior0.muPhi(dim.n_phi+dim.n_theta+1:end);
            posterior.SigmaX0 = ...
                posterior0.SigmaPhi(dim.n_phi+dim.n_theta+1:end,...
        ...
        if flag && isequal(options0.g_fname,@VBA_smoothNLSS)
            ...
            for t=2:dim.n_t
                ...
                posterior.SigmaX.current{t} = ...
                    ...
                    posterior.SigmaX.current{t};
                suffStat.SX = suffStat.SX + ...
                    0.5*VBA_logDet(posterior.SigmaX.current{t});
