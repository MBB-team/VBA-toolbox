...
It may happen that proper model parameterization requires to constrain the sample moments of some parameter subsets. For example, one may wish to make sure that observation parameters $$\phi$$ have a zero sample mean (i.e. $$\overline{\phi}= 1/n \sum_i \phi_i =0$$) and unitary sample variance (i.e. $$1/n\sum_i \left(\phi_i-\overline{\phi}\right)^2 =1$$), where the $$\phi_i$$s are the relevant entries of the parameter vector $$\phi$$. This is useful, e.g., in the context of [(re-parameterized) covariance component models]({{ site.baseurl }}/wiki/generativeModels/#covariance-component-models). This kind of constraint can be easily enforced by zscoring the parameter vector $$\phi$$ prior to entering the native observation function. An example of such constraint, in the context of VBA inversion of DDM models, can be found [here](https://www.frontiersin.org/articles/10.3389/frai.2021.531316/full). 

> Note that this will induce some (minor) form of model [over-parameterization](https://stats.stackexchange.com/questions/413467/different-notions-of-over-parameterization). For example, setting the sample mean to zero effectively removes one degree of freedom, because any entry in the ensuing parameter subset has to be equal to (minus) the sum over the remaining parameter entries. Setting the sample variance to unity yields similar effects. Nevertheless, this does not imply that the model will not be invertible, nor that the model inversion will produce overfitting!
