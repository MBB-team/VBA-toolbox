...
> **Upper-left panel**: the distribution of correct classifications under the null (grey bars) is compared with the actual classification accuracy (red line). This serves to perform classical inference on classification accuracy (p-value). **Lower-left panel**: posterior distribution over the classification accuracy. This serves to perform Bayesian inference on classification accuracy (exceedance probability = P(r>0.5)). **Upper-right panel**: classifier weights estimates for each train fold. This can be used to eyball the estimation stability of classifier weights. **Lower-right panel**: Summary statistics of data classification.
