...
% in_session
in_sessions = struct();
in_sessions.f_fname = @f_Qlearn_Pessiglione2006_2Q_M1;
in_sessions.g_fname = @g_softmax_Pessiglione2006_2Q_M1;
in_sessions.dim = dim;
in_sessions.binomial = 1;

in_sessions.ind.theta =  [1]*ones(Nsessions_tot,1); % same theta for each session
in_sessions.ind.phi =  [1]*ones(Nsessions_tot,1);   % same phi for each session
